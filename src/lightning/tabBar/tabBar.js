...
export default class LightningTabBar extends LightningElement {
    ...
    set tabHeaders(tabHeaders = []) {
        ...
        let selectedTab = this._allTabs[0];
        if (this._selectedTab) {
            selectedTab = this._allTabs.find(
                (tab) => tab.value === this._selectedTab.value
            );
            if (!selectedTab) {
                selectedTab = this._allTabs[0];
            }
        }
        if (selectedTab) {
    ...
    selectTabByValue(tabValue) {
        this._selectTab(tabValue, { fireTabSelectEvent: true });
    }
